...
func TestEndToEndDeploymentScenario(t *testing.T) {
	...
	test_structure.RunTestStage(t, "validate", func() {
		...
		*/
		...
		//correcting fqdn
		dbFqdn := ("c-" + databaseId + ".rw.mdb.yandexcloud.net")
		fmt.Println(dbFqdn)
		...
		mysqlInfo := fmt.Sprintf("user:password@tcp(%s)/db?tls=custom", dbFqdn)
		...
		logger.Log(t, "Ping the DB with forced SSL")
		if err = db.Ping(); err != nil {
			logger.Logf(t, "Not allowed to ping %s as expected.", dbFqdn)
		} else {
			//t.Fatalf("Ping %v succeeded against the odds.", firstMysqlFqdn)
			fmt.Print("Successfully pinged the DB with forced SSL \n")
		}
